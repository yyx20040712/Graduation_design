...
class WuniNongsuoNode(NodeBase):
    ...
    def execute_sludge(
        ...
        result.add_check("池径 D >= 5", D >= 5, round(D, 1), ">= 5", "m")
        ...
        result.add_check(
            "固体负荷合理",
            30 <= q_solid_actual <= 60,
            round(q_solid_actual, 1),
            "30~60 (GB50014 §8.2.1)",
            "kgDS/(m²·d)",
        )
        ...
        result.add_dimension("固体负荷(实际)", round(q_solid_actual, 1), "kgDS/(m²·d)")
        result.add_dimension("浓缩时间", T_thicken, "h")
    ...
    def _vectorized_compute(cls, grid, flow, quality, fixed):
        ...
        dt = np.dtype(
            [
                ("D", np.float64),
                ("h_eff_calc", np.float64),
                ("H_total", np.float64),
                ("q_solid_actual", np.float64),
        ...
        arr["D"] = D
        arr["h_eff_calc"] = h_eff_calc
        arr["H_total"] = H_total
        arr["q_solid_actual"] = q_solid_actual
        arr["Q_wet_out"] = Q_wet_out
        arr["concrete_m3"] = concrete_m3
        arr["ok_D_min"] = ok_D_min
        arr["ok_solid_flux"] = ok_solid_flux
        arr["ok_h_eff"] = ok_h_eff
        arr["val_D_min"] = D
        arr["val_solid_flux"] = q_solid_actual
        arr["val_h_eff"] = h_eff_calc
        return arr
