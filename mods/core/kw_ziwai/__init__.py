...
class KwZiwaiNode(NodeBase):
    ...
    def calculate(self, flow: WaterFlow,
        ...
        result.add_check("渠内流速", 0.05 <= v_actual <= 0.7,
                         round(v_actual, 3), "0.05~0.7", "m/s")
    ...
    def _vectorized_compute(
        ...
        v_actual = Q_single / A_channel
        ok_v = (0.05 <= v_actual) & (v_actual <= 0.7)

        I_avg = P_lamp * N_layer * eta_geo * T_eff * k_total / (10.0 * A_channel)
        dose_per_row = I_avg * d_long / v_actual
