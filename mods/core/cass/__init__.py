...
class CASSNode(NodeBase):
    ...
    def _init_ports(self) -> None:
        ...
        self.output_ports = [
            ...
            Port(port_id=f"{self.node_id}-sludge", name="剩余污泥",
    ...
    def _default_params(cls) -> Dict[str, float]:
        return {
            "n": 4, "Ns": 0.08, "X_MLSS": 3500, "f": 0.75,
            "theta_c": 20, "Y": 0.6, "Kd20": 0.05, "theta_t": 1.04,
            "T_design": 12, "Tc": 6, "lam": 0.3, "H_max": 6.0,
            "h_super": 0.5, "r_selector": 0.15, "SVI": 120,
            "delta_H_safe": 0.5,
            "a_prime": 0.5, "b_prime": 0.12,
            "t_d": 1,
        }

    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ParamDef("池数", "n", value=4, default=4,
                     min_val=2, max_val=8, step=1, unit="座"),
            ParamDef("BOD负荷 Ns", "Ns", value=0.08, default=0.08,
                      min_val=0.05, max_val=0.15, step=0.01,
                     unit="kgBOD5/(kgMLSS·d)"),
            ParamDef("MLSS浓度 X", "X_MLSS", value=3500, default=3500,
                     min_val=2500, max_val=4500, step=100, unit="mg/L"),
            ParamDef("MLVSS/MLSS f", "f", value=0.75, default=0.75,
                     min_val=0.7, max_val=0.8, step=0.01, unit=""),
            ParamDef("设计污泥龄 θc", "theta_c", value=20, default=20,
                      min_val=10, max_val=30, step=1, unit="d"),
            ParamDef("产率系数 Y", "Y", value=0.6, default=0.6,
                     min_val=0.4, max_val=0.8, step=0.05, unit="kgVSS/kgBOD5"),
            ParamDef("设计水温 T", "T_design", value=12, default=12,
                     min_val=8, max_val=25, step=1, unit="°C"),
            ParamDef("工作周期 Tc", "Tc", value=6, default=6,
                     min_val=4, max_val=8, step=1, unit="h"),
            ParamDef("充水比 λ", "lam", value=0.3, default=0.3,
                     min_val=0.2, max_val=0.4, step=0.05, unit=""),
            ParamDef("最大有效水深", "H_max", value=6.0, default=6.0,
                      min_val=4.0, max_val=6.0, step=0.1, unit="m"),
            ParamDef("超高", "h_super", value=0.5, default=0.5,
                     min_val=0.5, max_val=0.5, step=0.1, unit="m"),
            ParamDef("选择区比例", "r_selector", value=0.15, default=0.15,
                     min_val=0.10, max_val=0.20, step=0.01, unit=""),
            ParamDef("SVI", "SVI", value=120, default=120,
                     min_val=80, max_val=150, step=5, unit="mL/g"),
            ParamDef("安全距离 ΔH", "delta_H_safe", value=0.5, default=0.5,
                     min_val=0.3, max_val=0.8, step=0.1, unit="m"),
        ]

    @classmethod
    def _default_removal_rates(cls) -> Dict[str, float]:
        return {"BOD5": 0.92, "COD": 0.88, "SS": 0.70,
                "NH3N": 0.90, "TN": 0.75, "TP": 0.65}
    ...
    def calculate(self, flow: WaterFlow,
        ...
        result.params = {
            "n": n, "Ns": Ns, "X_MLSS": X_MLSS, "f": f,
            "theta_c": theta_c_design, "Y": Y, "Kd20": Kd20,
            "theta_t": theta_t, "T_design": T_design, "Tc": Tc,
            "lam": lam, "H_max": H_max, "h_super": h_super,
            "r_selector": r_selector, "SVI": SVI,
            "delta_H_safe": delta_H_safe,
            "a_prime": a_prime, "b_prime": b_prime, "t_d": t_d,
        }
        ...
        result.add_check("长宽比 L/B", 4 <= ratio_LB <= 6,
                         round(ratio_LB, 2), "4~6", "")
        result.add_check("宽高比 B/H", 1 <= ratio_BH <= 2,
                         round(ratio_BH, 2), "1~2", "")
        ...
        result.add_check("安全距离 1.5~2.0m", safe_ok,
                         round(H_safe, 2), "1.5~2.0", "m")

        # ── (6b) 充水比一致性校核 (3-61)(3-62) — 放在 λ 调整之后 ──
        lam_check = Q_avg * Tc / (24 * n * V_eff_actual)
        lam_deviation = abs(lam_check - lam) / lam if lam > 0 else 0
        result.add_dimension("设计充水比 λ_design", round(lam, 3), "",
                             formula="λ_design = 用户设定值 (0.2~0.4)",
                             category="computed")
        result.add_dimension("实际充水比 λ_actual", round(lam_check, 3), "",
                             formula="λ_actual = Q_avg × Tc / (24 × n × V_eff)",
                             category="computed")
        result.add_check("充水比一致性 |λ_actual-λ_design|/λ_design < 15%",
                         lam_deviation < 0.15,
                         round(lam_deviation * 100, 1), "< 15%", "%偏差")
        ...
        Px_bio = (Y * Q_avg * (S0_kg - Se_kg)
                  - KdT * V_main * X_kg * f / theta_c_design)  # kgVSS/d
        ...
        result.add_check("污泥龄校核 θc'", theta_c_ok,
                         round(theta_c_prime, 1),
                         f"{theta_c_design}~30", "d")

        # ── (11b) 硝化污泥龄校核 (4-79 注) ──
        # 当需要满足氨氮完全硝化要求时,θc'一般不应小于15~30 d
        result.add_check("硝化污泥龄校核 θc'≥15d",
                         15 <= theta_c_prime <= 30,
                         round(theta_c_prime, 1), "15~30", "d")
        ...
        result.add_check("总需氧量 > 0", O2_total > 0,
                         round(O2_total, 1), "> 0", "kgO2/d")
        ...
        result.add_dimension("滗水器堰口长度 L_w", round(L_w, 2), "m",
                             formula="L_w = Q_h / (q_L × 3.6), q_L=25 L/(s·m)",
                             category="physical")
        result.add_check("堰口负荷 q_L 20~30 L/(s·m)",
                         20 <= q_L_actual <= 30,
                         round(q_L_actual, 1), "20~30", "L/(s·m)")
        ...
        result.add_dimension("实际污泥龄 θc'", round(theta_c_prime, 1), "d",
    ...
    def _vectorized_compute(
        ...
        lam = grid["lam"]
        H_max = grid["H_max"]
        f = fixed["f"]; Y = fixed["Y"]; Kd20 = fixed["Kd20"]
        theta_t = fixed["theta_t"]; T_design = fixed["T_design"]
        h_super = fixed["h_super"]; r_selector = fixed["r_selector"]
        SVI = fixed["SVI"]; delta_H_safe = fixed["delta_H_safe"]
        a_prime = fixed["a_prime"]; b_prime = fixed["b_prime"]
        t_d = fixed["t_d"]
        N = len(n)
        ...
        dtype = np.dtype([
            ("V_main", np.float64), ("V_main_single", np.float64),
            ("V_selector", np.float64), ("V_eff_actual", np.float64),
            ("L", np.float64), ("B", np.float64), ("H_max_out", np.float64),
            ("H_total", np.float64), ("H_decant", np.float64),
            ("H_sludge", np.float64), ("H_safe", np.float64),
            ("lam_out", np.float64), ("ratio_LB", np.float64), ("ratio_BH", np.float64),
            ("Ns_out", np.float64), ("theta_c_actual", np.float64),
            ("Px_bio", np.float64), ("Px_nbio", np.float64), ("Px_total", np.float64),
            ("O2_carbon", np.float64), ("O2_nitrification", np.float64),
            ("O2_denitrification", np.float64), ("O2_total", np.float64),
            ("Q_decant", np.float64), ("KdT", np.float64),
            ("L_w", np.float64), ("q_L_actual", np.float64),
            ("H", np.float64),
            ("concrete_m3", np.float64),
            ("ok_LB", np.bool_), ("ok_BH", np.bool_), ("ok_lam", np.bool_),
            ("ok_safe", np.bool_), ("ok_theta_c", np.bool_), ("ok_O2", np.bool_),
            ("ok_weir", np.bool_), ("ok_nitrification", np.bool_),
            ("val_LB", np.float64), ("val_BH", np.float64), ("val_lam", np.float64),
            ("val_safe", np.float64), ("val_theta_c", np.float64), ("val_O2", np.float64),
            ("val_weir", np.float64), ("val_nitrification", np.float64),
        ])
        result = np.empty(N, dtype=dtype)
        result["V_main"] = V_main; result["V_main_single"] = V_main_single
        result["V_selector"] = V_selector; result["V_eff_actual"] = V_eff_actual
        result["L"] = L; result["B"] = B; result["H_max_out"] = H_max
        result["H_total"] = H_total; result["H_decant"] = H_decant
        result["H_sludge"] = H_sludge; result["H_safe"] = H_safe
        result["lam_out"] = lam; result["ratio_LB"] = ratio_LB; result["ratio_BH"] = ratio_BH
        result["Ns_out"] = Ns;         result["theta_c_actual"] = theta_c_prime
        result["Px_bio"] = Px_bio; result["Px_nbio"] = Px_nbio; result["Px_total"] = Px_total
        result["O2_carbon"] = O2_carbon; result["O2_nitrification"] = O2_nitrification
        result["O2_denitrification"] = O2_denitrification; result["O2_total"] = O2_total
        result["Q_decant"] = Q_decant; result["KdT"] = KdT
        result["L_w"] = L_w; result["q_L_actual"] = q_L_actual
        result["concrete_m3"] = concrete_m3
        result["L"] = L  # standard field
        result["B"] = B  # standard field
        result["H"] = result["H_total"]  # standard field
        result["ok_LB"] = ok_LB; result["ok_BH"] = ok_BH; result["ok_lam"] = ok_lam
        result["ok_safe"] = ok_safe; result["ok_theta_c"] = ok_theta_c; result["ok_O2"] = ok_O2
        result["ok_weir"] = ok_weir; result["ok_nitrification"] = ok_nitrification
        result["val_LB"] = ratio_LB; result["val_BH"] = ratio_BH; result["val_lam"] = lam_deviation
        result["val_safe"] = H_safe; result["val_theta_c"] = theta_c_prime; result["val_O2"] = O2_total
        result["val_weir"] = q_L_actual; result["val_nitrification"] = theta_c_prime
        return result
