...
class WuniShusongNode(NodeBase):
    ...
    def _default_params(cls) -> Dict[str, float]:
        return {
            "n_pumps": 2, "Q_pump": 20.0,
            "H_pump": 15.0, "v_pipe": 1.5,
            "L_pipe": 80.0,
        }
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ...
            ParamDef("单泵流量", "Q_pump", value=20.0, default=20.0,
                     min_val=5, max_val=100, step=5, unit="m³/h"),
            ParamDef("扬程", "H_pump", value=15.0, default=15.0,
                     min_val=5, max_val=50, step=1, unit="m"),
            ParamDef("管道流速", "v_pipe", value=1.5, default=1.5,
                     min_val=1.0, max_val=2.5, step=0.1, unit="m/s",
                     description="污泥管道设计流速 (≥1.0m/s)"),
            ParamDef("出水管长度", "L_pipe", value=80.0, default=80.0,
    ...
    def execute_sludge(self, sludge: SludgeFlow) -> Tuple[Optional[NodeResult], SludgeFlow]:
        ...
        result.add_check("管道流速合理", 1.0 <= v_pipe <= 2.5,
                         v_pipe, "1.0~2.5", "m/s")
    ...
    def _vectorized_compute(cls, grid, flow, quality, fixed):
        ...
        # 出水管水头损失 (Manning)
        n_rough = 0.013; L_pipe = fixed.get("L_pipe", 80.0)
        R_hyd = D_actual / 4.0
