...
class KwTiaojiechiNode(NodeBase):
    ...
    def calculate(self, flow: WaterFlow, quality: WaterQuality) -> NodeResult:
        ...
        result.add_check(
            "实际 HRT", 6.0 <= HRT_actual <= 12.0, round(HRT_actual, 2), "6~12", "h"
        )
        ...
        result.add_dimension(
            "池数", n, "座", formula="n = 4~8 (步长2)", category="physical"
        )
        result.add_dimension(
            "单池长度 L", L, "m", formula="L = ceil(A₁/B, 0.5m)", category="physical"
        )
        result.add_dimension(
            "单池宽度 B",
            B,
        ...
        result.add_dimension(
            "池底坡度 i", slope, "", formula="i = 设计取值 (≥0.01)", category="physical"
        )
        result.add_dimension(
            "单池有效容积",
            round(V_actual, 1),
    ...
    def _vectorized_compute(
        ...
        if flow.Q_design <= 0:
            dtype = np.dtype(
                [
                    ...
                    ("dry_sludge", np.float64),
                    ("wet_sludge", np.float64),
                    ("V_sludge_needed", np.float64),
                    ("V_pit_available", np.float64),
                    ("u_s_mm_s", np.float64),
                    ("D_out_mm", np.float64),
                    ("q_weir", np.float64),
                    ("H", np.float64),
                    ...
                    ("ok_pit_vol", np.bool_),
                    ("ok_weir_load", np.bool_),
                    ("val_LB_ratio", np.float64),
                    ("val_HRT_actual", np.float64),
                    ("val_pit_vol", np.float64),
                    ("val_weir_load", np.float64),
                ]
            )
        ...
        wet_sludge = np.where(
            P_sludge < 1, dry_sludge / ((1 - P_sludge) * rho_sludge), 0.0
        )
        ...
        D_out_theory = np.where(
            v_out > 0, np.sqrt(4 * Q_out_m3s / (np.pi * v_out)), 0.0
        )
        ...
        result["dry_sludge"] = dry_sludge
        result["wet_sludge"] = wet_sludge
        result["V_sludge_needed"] = V_sludge_needed
        result["V_pit_available"] = V_pit_available
        result["u_s_mm_s"] = u_s_mm_s
        ...
        result["ok_LB_ratio"] = ok_LB
        result["ok_HRT_actual"] = ok_HRT
        result["ok_pit_vol"] = ok_pit
        result["ok_weir_load"] = ok_weir
        result["val_LB_ratio"] = ratio_actual
        result["val_HRT_actual"] = HRT_actual
        result["val_pit_vol"] = V_pit_available
        result["val_weir_load"] = q_weir
        return result
