...
class GaomiduNode(NodeBase):
    ...
    def _default_params(cls) -> Dict[str, float]:
        return {
            "n": 4, "t_mix": 1.0, "t_floc": 10.0,
            "R_sludge": 0.03, "q_surf": 8.0,
            "L_tube": 1.0, "alpha_tube": 60.0,
            "h_clear": 1.0, "h_dist": 1.5, "h_super": 0.5,
            "t_thicken": 1.5, "P_out": 0.96,
            "D_PAC": 40.0, "k_PAC": 0.5,
            "G_mix": 750.0, "G_floc": 75.0,
            "X_r_assumed": 15.0,
        }
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ParamDef("池数", "n", value=4, default=4,
                     min_val=2, max_val=6, step=1, unit="座"),
            ParamDef("混合时间", "t_mix", value=1.0, default=1.0,
                     min_val=0.5, max_val=2.0, step=0.1, unit="min"),
            ParamDef("絮凝时间", "t_floc", value=10.0, default=10.0,
                     min_val=8.0, max_val=15.0, step=0.5, unit="min"),
            ParamDef("污泥回流比", "R_sludge", value=0.03, default=0.03,
                     min_val=0.01, max_val=1.80, step=0.01, unit=""),
            ParamDef("斜管区表面负荷", "q_surf", value=8.0, default=8.0,
                     min_val=6.0, max_val=12.0, step=0.5,
                     unit="m³/(m²·h)"),
            ParamDef("斜管长度", "L_tube", value=1.0, default=1.0,
                     min_val=0.8, max_val=1.5, step=0.1, unit="m"),
            ParamDef("斜管倾角", "alpha_tube", value=60, default=60,
                     min_val=55, max_val=65, step=1, unit="°"),
            ParamDef("PAC投加量", "D_PAC", value=40, default=40,
                     min_val=15, max_val=80, step=5, unit="mg/L"),
        ]

    @classmethod
    def _default_removal_rates(cls) -> Dict[str, float]:
        return {"SS": 0.90, "COD": 0.60, "BOD5": 0.20,
                "NH3N": 0.0, "TN": 0.0, "TP": 0.85}
    ...
    def calculate(self, flow: WaterFlow,
        ...
        result.add_dimension("混合区容积", round(V_mix, 2), "m³",
                             formula="V_mix = Q_max × t_mix",
                             category="physical")
        result.add_dimension("混合区功率 P_mix", round(P_mix, 1), "W",
                             formula="P_mix = G_mix² × μ × V_mix",
                             category="computed")
        result.add_check("混合区 G_mix 500~1000 s⁻¹",
                         500 <= G_mix <= 1000,
                         round(G_mix, 0), "500~1000", "s⁻¹")
        ...
        result.add_dimension("絮凝区容积", round(V_floc, 2), "m³",
                             formula="V_floc = (1+R)×Q_max × t_floc",
                             category="physical")
        result.add_dimension("絮凝区功率 P_floc", round(P_floc, 1), "W",
                             formula="P_floc = G_floc² × μ × V_floc",
                             category="computed")
        result.add_check("絮凝区 G_floc 50~100 s⁻¹",
                         50 <= G_floc <= 100,
                         round(G_floc, 0), "50~100", "s⁻¹")
        ...
        result.add_dimension("沉淀区面积", round(A_settle, 1), "m²",
                             ...
                             category="physical")
        result.add_dimension("斜管轴向流速 v₀", round(v_axial, 2), "mm/s",
                             formula="v₀ = q_surf / (3600 × sinθ)",
                             category="computed")
        result.add_check("斜管轴向流速 ≤ 5 mm/s",
                         v_axial <= 5.0,
                         round(v_axial, 2), "≤ 5", "mm/s")
        ...
        result.add_dimension("池宽 B", B_pool, "m",
                             ...
                             category="physical")
        result.add_check("长宽比 L/B 1~2",
                         1.0 <= ratio_LB <= 2.0,
                         round(ratio_LB, 2), "1~2", "")
        ...
        result.add_dimension("SS去除干污泥 W_SS(总)", round(S_dry_SS_total, 1), "kg/d",
                             formula="W_SS = Q_d × (C₀-Cₑ) × 10⁻³",
                             category="computed")
        result.add_dimension("化学干污泥 W_chem(总)", round(S_dry_chem_total, 1), "kg/d",
                             formula="W_chem = Q_d × D_PAC × κ_PAC × 10⁻³",
                             category="computed")
        result.add_dimension("总干污泥 W_s(总)", round(S_dry_total, 1), "kg/d",
                             formula="W_s = W_SS + W_chem",
                             category="computed")
        result.add_dimension("单池干污泥 W_s(单池)", round(S_dry_per, 1), "kg/d",
                             formula="W_s(单池) = W_s(总) / n",
                             category="computed")
        result.add_dimension("日湿污泥量 V_s,in(总)", round(V_sludge_wet_total, 1), "m³/d",
        ...
        result.add_dimension("浓缩区高度 h_thick(单池)", round(h_thicken, 2), "m",
                             formula="h_thick = V_thicken(单池) / A(单池)",
                             category="physical")
        result.add_dimension("固体通量 G(单池)", round(solid_flux, 1), "kgDS/(m²·d)",
                             formula="G = W_s(单池) / A(单池)",
                             category="computed")
        result.add_check("固体通量 G ≤ 150 kgDS/(m²·d)",
                         solid_flux <= 150,
                         round(solid_flux, 1), "≤ 150", "kgDS/(m²·d)")

        # 回流污泥浓度校核 X_r = W_s(单池) / Q_r  (4-92)
        Q_r_daily = Q_r * 86400.0  # m³/d (单池回流量)
        X_r_calc = S_dry_per / Q_r_daily if Q_r_daily > 0 else 0  # kg/m³ = g/L
        result.add_dimension("回流污泥浓度 X_r(单池)", round(X_r_calc, 1), "g/L",
                             formula="X_r = W_s(单池) / Q_r",
                             category="computed")
        result.add_check("回流污泥浓度 X_r ≥ 0 g/L",
                         X_r_calc >= 0,
                         round(X_r_calc, 1), "≥ 0", "g/L")
        if X_r_calc < 1.0:
            result.add_warning(
        ...
        result.add_dimension("出水堰总长 L_w", round(weir_len, 1), "m",
                             formula="L_w = 2×(L+B)×2 (双侧堰)",
                             category="physical")
        result.add_dimension("堰负荷 q_堰", round(q_weir, 2), "L/(s·m)",
                             formula="q_堰 = Q_max(L/s) / L_w(m)",
                             category="computed")
        result.add_check("堰负荷 1.5~2.9 L/(s·m)",
                         1.5 <= q_weir <= 2.9,
                         round(q_weir, 2), "1.5~2.9", "L/(s·m)")

        # ── 汇总 ──
        result.add_dimension("池数", n, "座")
        result.add_dimension("PAC日耗量", round(D_PAC * flow.Q_avg_daily / 1000, 1), "kg/d")

        # ── 污泥输出 (SLUDGE 端口, 化学污泥, 全厂总量) ──
        self._sludge_output = SludgeFlow(
            Q_wet=V_sludge_wet_total, DS=S_dry_total,
            P_moisture=P_in, VS_ratio=0.40,
        )
    ...
    def _vectorized_compute(
        ...
        if flow.Q_design <= 0:
            dtype = np.dtype([
                ("V_mix", np.float64), ("V_floc", np.float64), ("A_settle", np.float64),
                ("L_pool", np.float64), ("B_pool", np.float64),
                ("P_mix", np.float64), ("P_floc", np.float64),
                ("v_axial", np.float64), ("H_total", np.float64),
            ("S_dry_SS_total", np.float64), ("S_dry_chem_total", np.float64), ("S_dry_total", np.float64),
                ("h_thicken", np.float64), ("solid_flux", np.float64),
                ("X_r_calc", np.float64), ("weir_len", np.float64), ("q_weir", np.float64),
                ("L", np.float64), ("B", np.float64), ("H", np.float64),
                ("concrete_m3", np.float64),
                ("ok_axial_v", np.bool_), ("ok_solid_flux", np.bool_),
                ("ok_G_mix", np.bool_), ("ok_G_floc", np.bool_),
                ("ok_LB", np.bool_), ("ok_X_r", np.bool_), ("ok_weir", np.bool_),
                ("val_axial_v", np.float64), ("val_solid_flux", np.float64),
                ("val_G_mix", np.float64), ("val_G_floc", np.float64),
                ("val_LB", np.float64), ("val_X_r", np.float64), ("val_weir", np.float64),
            ])
        ...
        P_in = 0.99
        V_sludge_wet_total = np.where(P_in < 1, S_dry_total / ((1 - P_in) * 1000.0), 0.0)
        # 单池量
        S_dry_per = S_dry_total / n
        V_sludge_wet_per = V_sludge_wet_total / n
        ...
        result["V_mix"] = V_mix; result["V_floc"] = V_floc; result["A_settle"] = A_settle
        result["L_pool"] = L_pool; result["B_pool"] = B_pool
        result["P_mix"] = P_mix; result["P_floc"] = P_floc
        result["v_axial"] = v_axial; result["H_total"] = H_total
        result["S_dry_SS_total"] = S_dry_SS_total; result["S_dry_chem_total"] = S_dry_chem_total
        result["S_dry_total"] = S_dry_total; result["h_thicken"] = h_thicken
        result["solid_flux"] = solid_flux
        result["X_r_calc"] = X_r_calc; result["weir_len"] = weir_len; result["q_weir"] = q_weir
        result["concrete_m3"] = concrete_m3
        result["L"] = result["L_pool"]; result["B"] = result["B_pool"]; result["H"] = result["H_total"]
        result["ok_axial_v"] = ok_axial; result["ok_solid_flux"] = ok_flux
        result["ok_G_mix"] = ok_G_mix; result["ok_G_floc"] = ok_G_floc
        result["ok_LB"] = ok_LB; result["ok_X_r"] = ok_X_r; result["ok_weir"] = ok_weir
        result["val_axial_v"] = v_axial; result["val_solid_flux"] = solid_flux
        result["val_G_mix"] = np.full(N, G_mix); result["val_G_floc"] = np.full(N, G_floc)
        result["val_LB"] = ratio_LB; result["val_X_r"] = X_r_calc; result["val_weir"] = q_weir
        return result
