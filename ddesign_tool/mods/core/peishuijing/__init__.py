...
from models.base import (
    ...
    ParamDef, Port, PortType, GRAVITY, PI,
)
...
class PeishuijingNode(NodeBase):
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ParamDef("出水方向数", "n_out", value=2, default=2,
                     min_val=1, max_val=6, step=1, unit="个"),
            ParamDef("堰上水头", "h_weir", value=0.3, default=0.3,
                     min_val=0.1, max_val=0.5, step=0.05, unit="m"),
            ParamDef("水力停留时间", "HRT", value=3.0, default=3.0,
                     min_val=1.0, max_val=10.0, step=1.0, unit="min",
                     description="GB50014: 配水井 1~10min"),
            ParamDef("有效水深", "h_eff", value=2.5, default=2.5,
                     min_val=1.5, max_val=4.0, step=0.5, unit="m"),
            ParamDef("超高", "h_super", value=0.5, default=0.5,
                     min_val=0.3, max_val=1.0, step=0.1, unit="m"),
        ]
    ...
    def calculate(self, flow: WaterFlow, quality: WaterQuality) -> NodeResult:
        ...
        m = 0.42
        b_weir = Q_max / 3600.0 / (n_out * m * math.sqrt(2 * GRAVITY) * h_weir ** 1.5) if h_weir > 0 else 0  # m
        ...
        result.params = {"n_out": n_out, "h_weir": h_weir, "HRT": HRT, "h_eff": h_eff, "h_super": h_super}
        result.add_dimension("出水方向数", n_out, "个")
        result.add_dimension("有效容积 V", round(V, 2), "m3",
                             formula="V = Q_max × HRT / 60")
        result.add_dimension("堰上水头", h_weir, "m")
        result.add_dimension("井径 D", D, "m",
                             formula="D = max(D_HRT, D_weir, 2.0), ceil 0.5m")
        result.add_dimension("总高度 H", round(H_total, 2), "m",
                             formula="H = h_eff + h_super")
        result.add_dimension("有效水深", h_eff, "m")
        result.add_check("水力停留时间 1~10 min", 1.0 <= HRT <= 10.0, round(HRT, 1), "1~10", "min")
        result.add_check("有效水深 1.5~4.0 m", 1.5 <= h_eff <= 4.0, round(h_eff, 1), "1.5~4.0", "m")
    ...
    def _vectorized_compute(cls, grid: dict, flow: WaterFlow,
        ...
        # 堰长法
        b_weir = np.where(h_weir > 0,
            Q_max / 3600.0 / (n_out * m * np.sqrt(2 * GRAVITY) * h_weir ** 1.5), 0.0)
        D_weir = b_weir * 1.5
        ...
        dtype = np.dtype([("V", np.float64), ("D", np.float64), ("H_total", np.float64),
                          ("H", np.float64), ("concrete_m3", np.float64),
                          ("ok_HRT", np.bool_), ("ok_h_eff", np.bool_), ("ok_D", np.bool_),
                          ("val_HRT", np.float64), ("val_h_eff", np.float64), ("val_D", np.float64)])
        arr = np.empty(N, dtype=dtype)
        arr["V"] = V; arr["D"] = D; arr["H_total"] = H_total; arr["H"] = H_total
        arr["concrete_m3"] = concrete_m3
        arr["ok_HRT"] = ok_HRT; arr["ok_h_eff"] = ok_h_eff; arr["ok_D"] = ok_D
        arr["val_HRT"] = HRT; arr["val_h_eff"] = h_eff; arr["val_D"] = D
        return arr
