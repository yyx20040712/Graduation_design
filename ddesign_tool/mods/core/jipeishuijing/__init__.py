...
class JipeishuijingNode(NodeBase):
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ...
            ParamDef("有效水深", "h_eff", value=3.0, default=3.0,
                     min_val=2.0, max_val=5.0, step=0.5, unit="m"),
            ParamDef("超高", "h_super", value=0.5, default=0.5,
                     min_val=0.3, max_val=1.0, step=0.1, unit="m"),
            ParamDef("水力停留时间", "HRT", value=5.0, default=5.0,
                     min_val=3.0, max_val=15.0, step=1.0, unit="min"),
        ]
    ...
    def calculate(self, flow: WaterFlow, quality: WaterQuality) -> NodeResult:
        ...
        result.params = {"n_out": n_out, "h_weir": h_weir, "h_eff": h_eff,
                         "h_super": h_super, "HRT": HRT}
        ...
        result.add_check("水力停留时间 3~15 min", 3.0 <= HRT <= 15.0, round(HRT, 1), "3~15", "min")
        result.add_check("有效水深 2.0~5.0 m", 2.0 <= h_eff <= 5.0, round(h_eff, 1), "2.0~5.0", "m")
        return result
    ...
    def _vectorized_compute(cls, grid: dict, flow: WaterFlow,
        ...
        dtype = np.dtype([("V", np.float64), ("L", np.float64), ("B", np.float64),
                          ("H_total", np.float64), ("H", np.float64), ("concrete_m3", np.float64),
                          ("ok_HRT", np.bool_), ("ok_h_eff", np.bool_),
                          ("val_HRT", np.float64), ("val_h_eff", np.float64)])
        result = np.empty(N, dtype=dtype)
        result["V"] = V; result["L"] = L; result["B"] = B
        result["H_total"] = H_total; result["H"] = H_total
        result["concrete_m3"] = concrete_m3
        result["ok_HRT"] = ok_HRT; result["ok_h_eff"] = ok_h_eff
        result["val_HRT"] = HRT; result["val_h_eff"] = h_eff
        return result
