...
class WuniBengzhanNode(NodeBase):
    ...
    def calculate(self, flow, quality) -> NodeResult:
        ...
        result.add_dimension("单泵功率", round(float(r["P_motor"]), 1), "kW")
        result.add_dimension(
            "总装机功率", round(float(r["P_motor"]) * (n_pumps + 1), 1), "kW",
        )
        result.add_check(
            "泵送能力充足", bool(r["ok_capacity"]),
            round(float(r["val_capacity"]), 2), ">= 0", "m³/h",
        )
        result.add_check(
            "管道流速经济", bool(r["ok_v_pipe"]),
    ...
    def execute_sludge(
        ...
        result.add_check(
            "管道流速经济", 1.0 <= v_pipe <= 2.0, v_pipe, "1.0~2.0 (GB50014)", "m/s"
        )
