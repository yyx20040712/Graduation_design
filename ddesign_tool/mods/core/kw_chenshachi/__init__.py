...
class KwChenshachiNode(NodeBase):
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ParamDef("格数 n", "n", value=2, default=2,
                     min_val=2, max_val=8, step=1, unit="格",
                     description="沉砂池格数,≥2"),
            ParamDef("水平流速 v", "v_h", value=0.22, default=0.22,
                     min_val=0.20, max_val=0.25, step=0.01, unit="m/s",
                     description="最大流量时水平流速 0.20~0.25m/s"),
            ParamDef("停留时间 t", "t_stay", value=50, default=50,
                     min_val=30, max_val=60, step=5, unit="s",
                     description="≥30s,一般30~60s"),
            ParamDef("有效水深 H", "h_eff", value=0.8, default=0.8,
                     min_val=0.25, max_val=1.0, step=0.05, unit="m",
                     description="≤1.2m,一般0.25~1.0m"),
            ParamDef("超高", "h_super", value=0.3, default=0.3,
                     min_val=0.3, max_val=0.5, step=0.1, unit="m",
                     description="≥0.3m"),
            ParamDef("池底坡度 i", "slope", value=0.02, default=0.02,
                     min_val=0.01, max_val=0.02, step=0.005, unit="-",
                     description="0.01~0.02"),
            ParamDef("砂斗倾角 α", "hopper_angle", value=55, default=55,
                     min_val=55, max_val=60, step=1, unit="°",
                     description="≥55°,一般55°~60°"),
            ParamDef("砂斗下口宽 b₂", "hopper_bottom", value=0.4, default=0.4,
                      min_val=0.3, max_val=0.6, step=0.1, unit="m"),
            ParamDef("沉砂量系数 κ", "kappa", value=0.03, default=0.03,
                     min_val=0.03, max_val=0.05, step=0.01, unit="L/m³",
                     description="矿井水 0.03~0.05 L/m³"),
            ParamDef("贮砂时间 T", "T_sand", value=2, default=2,
                     min_val=1, max_val=3, step=1, unit="d",
                     description="一般按2d计"),
            ParamDef("沉砂含水率", "P_sand", value=0.60, default=0.60,
                     min_val=0.55, max_val=0.65, step=0.05, unit="-"),
            ParamDef("沉砂密度", "rho_sand", value=1500, default=1500,
                     min_val=1400, max_val=1600, step=50, unit="kg/m³"),
            ParamDef("设计堰负荷", "q_weir_design", value=2.5, default=2.5,
                     min_val=1.5, max_val=2.9, step=0.1, unit="L/(s·m)",
    ...
    def calculate(self, flow: WaterFlow,
        ...
        result.params = {
            "n": n, "v_h": v_h, "t_stay": t_stay,
            "h_eff": h_eff, "h_super": h_super, "slope": slope,
            "hopper_angle": hopper_angle, "hopper_bottom": hopper_bottom,
            "kappa": kappa, "T_sand": T_sand,
            "P_sand": P_sand, "rho_sand": rho_sand,
        ...
        result.add_check("水平流速 0.15~0.30 m/s",
                         0.15 <= v_actual <= 0.30,
                         round(v_actual, 3), "0.15~0.30", "m/s")
        if v_actual < 0.15:
        ...
        # ── 有效水深校核 H ≤ 1.2m ──
        result.add_check("有效水深 H ≤ 1.2m",
                         h_eff <= 1.2,
                         round(h_eff, 2), "≤ 1.2", "m")

        # ── 单格宽度校核 B ≥ 0.6m ──
        result.add_check("单格宽度 B ≥ 0.6m",
                         B >= 0.6,
                         round(B, 2), "≥ 0.6", "m")
        ...
        if h_eff > 0:
            ratio_BH = B / h_eff
            result.add_check("宽深比 B/H 1.0~2.0",
                             1.0 <= ratio_BH <= 2.0,
                             round(ratio_BH, 2), "1.0~2.0", "")
        ...
        result.add_check("砂斗容积足够",
                         hopper_ok,
                         round(V_hopper_total, 2),
                         f">= {round(V_hopper_needed_total, 2)}", "m³")
        if not hopper_ok:
            result.add_warning(
        ...
        result.add_check("堰口负荷 ≤ 10 L/(s·m)",
                         q_weir_actual <= 10,
                         round(q_weir_actual, 2), "≤ 10", "L/(s·m)")
        if q_weir_actual > 10:
            result.add_warning(
        ...
        result.add_dimension("排砂管最小管径", D_discharge, "mm",
                             formula="D_pipe ≥ 200mm (重力排砂)",
                             category="physical")

        # ── 组装结果 ──
        result.add_dimension("格数", n, "格",
                             formula="n ≥ 2",
                             category="physical")
        result.add_dimension("池长 L", L, "m",
                             formula="L = v × t, ceil 0.5m",
                             category="physical")
        result.add_dimension("单格宽度 B", B, "m",
                             formula="B = A₁ / H, ceil 0.1m, ≥0.6m",
                             category="physical")
        result.add_dimension("有效水深 H", h_eff, "m",
                             formula="H ≤ 1.2m (规范强制)",
                             category="physical")
        result.add_dimension("砂斗深度 h_斗", round(h_hopper, 2), "m",
                             formula="h_斗 = (B-b₂)/2 × tan(α)",
                             category="physical")
        result.add_dimension("总高度 H_t", H_rounded, "m",
                             formula="H_t = H + h_斗 + h_free",
                             category="physical")
        result.add_dimension("砂斗个数/格", n_hoppers_per_cell, "个",
                             formula="n_hoppers = L / B",
                             category="physical")
        result.add_dimension("砂斗总数", n_hoppers_per_cell * n, "个",
                             category="physical")
        result.add_dimension("单斗容积", round(V_hopper_single, 2), "m³",
                             formula="V_斗 = h/3×(b₁²+b₂²+b₁·b₂)",
                             category="physical")
        result.add_dimension("砂斗总容积", round(V_hopper_total, 2), "m³",
                             formula="V_total = V_斗 × n_hoppers × n",
                             category="physical")
        result.add_dimension("需砂斗容积", round(V_hopper_needed_total, 2), "m³",
                             formula="V_needed = V_sand,daily × T_sand",
                             category="computed")
        result.add_dimension("设计水平流速", v_h, "m/s",
                             formula="v = 设计取值 0.20~0.25",
                             category="computed")
        result.add_dimension("实际水平流速", round(v_actual, 3), "m/s",
                             formula="v_actual = Q_max/(n×B×H)",
                             category="computed")
        result.add_dimension("停留时间", t_stay, "s",
                             formula="t = 设计取值 30~60s",
                             category="computed")
        result.add_dimension("池底坡度", slope, "",
                             category="physical")
        result.add_dimension("日产砂体积", round(V_sand_daily, 3), "m³/d",
                             formula="V_sand = Q_d × κ/1000",
                             category="computed")
        result.add_dimension("需堰长 L_weir", round(L_weir_required, 1), "m",
                             formula="L_weir = Q_max×1000/q_weir",
                             category="computed")
        result.add_dimension("可用堰长", round(L_weir_available, 1), "m",
                             formula="L_avail = n × B",
                             category="physical")
        result.add_dimension("实际堰负荷", round(q_weir_actual, 2), "L/(s·m)",
                             formula="q_actual = Q_max×1000/L_avail",
                             category="computed")

        # 概算用
        result.add_dimension("沉砂池总面积", round(L * B * n, 1), "m²",
                             category="physical")
        result.add_dimension("混凝土量估算",
                           round(L * B * H_rounded * n * 1.3, 1), "m³",
                             category="physical")
    ...
    def _vectorized_compute(cls, grid: dict, flow: "WaterFlow",
        ...
        if flow.Q_design <= 0:
            dtype = np.dtype([
                ...
                ("H_total", np.float64), ("h_hopper", np.float64),
                ("V_hopper_single", np.float64), ("V_hopper_total", np.float64),
                ("V_hopper_needed", np.float64), ("V_sand_daily", np.float64),
                ("n_hoppers_per_cell", np.int32),
                ("v_actual", np.float64), ("ratio_BH", np.float64),
                ("L_weir_req", np.float64), ("L_weir_avail", np.float64),
                ("q_weir_actual", np.float64),
                ("H", np.float64), ("area_total", np.float64), ("concrete_m3", np.float64),
                ("ok_v", np.bool_), ("ok_H", np.bool_), ("ok_B", np.bool_),
                ("ok_BH", np.bool_), ("ok_hopper", np.bool_), ("ok_weir", np.bool_),
                ("val_v", np.float64), ("val_H", np.float64), ("val_B", np.float64),
                ("val_BH", np.float64), ("val_hopper", np.float64), ("val_weir", np.float64),
            ])
        ...
        result["H_total"] = H_total; result["h_hopper"] = h_hopper
        ...
        result["v_actual"] = v_actual; result["ratio_BH"] = ratio_BH
        result["L_weir_req"] = L_weir_req; result["L_weir_avail"] = L_weir_avail
        result["q_weir_actual"] = q_weir_actual
        result["H"] = result["H_total"]
        result["area_total"] = area_total; result["concrete_m3"] = concrete_m3
        result["ok_v"] = ok_v; result["ok_H"] = ok_H; result["ok_B"] = ok_B
        result["ok_BH"] = ok_BH; result["ok_hopper"] = ok_hopper; result["ok_weir"] = ok_weir
        result["val_v"] = v_actual; result["val_H"] = h_eff; result["val_B"] = B
        result["val_BH"] = ratio_BH; result["val_hopper"] = V_hopper_total
