...
class PeishuiquNode(NodeBase):
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ParamDef("渠宽 B", "B_channel", value=0.8, default=0.8,
                     min_val=0.5, max_val=3.0, step=0.1, unit="m"),
            ParamDef("有效水深 h_eff", "h_eff", value=1.5, default=1.5,
                     min_val=0.5, max_val=3.0, step=0.1, unit="m"),
            ParamDef("超高 h_super", "h_super", value=0.4, default=0.4,
                     min_val=0.3, max_val=0.8, step=0.1, unit="m"),
            ParamDef("出水口数", "n_out", value=4, default=4,
                     min_val=2, max_val=10, step=1, unit="个"),
        ]
    ...
    def calculate(self, flow: WaterFlow, quality: WaterQuality) -> NodeResult:
        ...
        result.params = {"B_channel": B, "h_eff": h_eff, "h_super": h_super, "n_out": n_out}
        result.add_dimension("渠宽 B", B, "m",
                             formula="B = 设计取值 (0.5~3.0m)")
        result.add_dimension("有效水深 h_eff", h_eff, "m",
                             formula="h_eff = 设计取值 (0.5~3.0m)")
        result.add_dimension("过水面积 A", round(A, 2), "m²",
                             formula="A = B × h_eff")
        result.add_dimension("实际流速 v_actual", round(v_actual, 3), "m/s",
                             formula="v_actual = Q / A")
        result.add_dimension("渠长 L", L, "m",
                             formula="L = v_actual × 60 (停留1min)")
        result.add_dimension("总高度 H", round(H_total, 2), "m",
                             formula="H = h_eff + h_super")
        result.add_dimension("出水口数", n_out, "个")
        result.add_check("渠内流速 0.3~1.2 m/s", 0.3 <= v_actual <= 1.2,
                         round(v_actual, 3), "0.3~1.2", "m/s")
        result.add_check("有效水深 0.5~3.0 m", 0.5 <= h_eff <= 3.0,
                         round(h_eff, 1), "0.5~3.0", "m")
        result.add_check("渠宽 0.5~3.0 m", 0.5 <= B <= 3.0,
                         round(B, 1), "0.5~3.0", "m")
        return result
    ...
    def _vectorized_compute(cls, grid: dict, flow: WaterFlow,
        ...
        dtype = np.dtype([("B", np.float64), ("h_eff", np.float64),
                          ("A", np.float64), ("v_actual", np.float64),
                          ("L", np.float64), ("H_total", np.float64),
        ...
        arr["B"] = B; arr["h_eff"] = h_eff; arr["A"] = A
        arr["v_actual"] = v_actual; arr["L"] = L; arr["H_total"] = H_total
        arr["H"] = H_total; arr["concrete_m3"] = concrete_m3
        arr["ok_v"] = ok_v; arr["ok_h"] = ok_h; arr["ok_B"] = ok_B
        arr["val_v"] = v_actual; arr["val_h"] = h_eff; arr["val_B"] = B
        return arr
