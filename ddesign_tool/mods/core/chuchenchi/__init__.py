...
class ChuchenchiNode(NodeBase):
    ...
    def _default_params(cls) -> Dict[str, float]:
        return {
            "n": 2, "q_prime": 2.0, "T_settle": 1.5,
            "h1": 0.3, "h3": 0.3, "i_slope": 0.05,
            "R1": 1.8, "R2": 0.8, "h5": 1.5,
            "P_sludge": 0.96, "T_sludge": 2, "v_center": 0.3,
        }
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ...
            ParamDef("表面负荷 q'", "q_prime", value=2.0, default=2.0,
                     min_val=1.5, max_val=3.0, step=0.1,
                     unit="m³/(m²·h)"),
            ParamDef("沉淀时间 T", "T_settle", value=1.5, default=1.5,
                     min_val=1.0, max_val=2.0, step=0.1, unit="h"),
            ParamDef("超高 h1", "h1", value=0.3, default=0.3,
                     min_val=0.3, max_val=0.5, step=0.1, unit="m"),
            ParamDef("缓冲层 h3", "h3", value=0.3, default=0.3,
                     min_val=0.3, max_val=0.5, step=0.1, unit="m"),
            ParamDef("池底坡度 i", "i_slope", value=0.05, default=0.05,
                     min_val=0.03, max_val=0.08, step=0.01, unit="-"),
            ParamDef("泥斗上口半径 R1", "R1", value=1.8, default=1.8,
                     min_val=1.5, max_val=2.0, step=0.1, unit="m"),
            ParamDef("泥斗下口半径 R2", "R2", value=0.8, default=0.8,
                     min_val=0.6, max_val=1.0, step=0.1, unit="m"),
            ParamDef("泥斗高度 h5", "h5", value=1.5, default=1.5,
                     min_val=1.2, max_val=2.0, step=0.1, unit="m"),
            ParamDef("污泥含水率", "P_sludge", value=0.96, default=0.96,
                     min_val=0.95, max_val=0.97, step=0.01, unit="-"),
            ParamDef("排泥周期", "T_sludge", value=2, default=2,
                     min_val=1, max_val=4, step=1, unit="d",
                     description="贮泥时间"),
            ParamDef("中心管流速", "v_center", value=0.3, default=0.3,
                     min_val=0.2, max_val=0.5, step=0.05, unit="m/s"),
        ]

    @classmethod
    def _default_removal_rates(cls) -> Dict[str, float]:
        return {"SS": 0.50, "BOD5": 0.30, "COD": 0.30,
                "NH3N": 0.05, "TN": 0.05, "TP": 0.05}
    ...
    def calculate(self, flow: WaterFlow,
        ...
        result.params = {
            "n": n, "q_prime": q_prime, "T_settle": T_settle,
            "h1": h1, "h3": h3, "i_slope": i_slope,
            "R1": R1, "R2": R2, "h5": h5,
            "P_sludge": P_sludge, "T_sludge": T_sludge, "v_center": v_center,
        }
        ...
        # ── 校核表面负荷 (3-30) ──
        result.add_check("实际表面负荷 q'", 1.5 <= q_prime_actual <= 3.0,
                         round(q_prime_actual, 2), "1.5~3.0", "m³/(m²·h)")

        # ── (C) 有效水深 (3-31) ──
        h2 = q_prime_actual * T_settle
        ratio_Dh2 = D / h2 if h2 > 0 else 0

        result.add_check("有效水深 h2", 2.0 <= h2 <= 4.0,
                         round(h2, 2), "2.0~4.0", "m")
        result.add_check("径深比 D/h2", 6 <= ratio_Dh2 <= 12,
        ...
        result.add_check("污泥区容积足够", V_total_storage >= V_sludge,
                         round(V_total_storage - V_sludge, 2), ">= 0", "m³")
        ...
        result.add_check("堰负荷", q_weir <= 2.9,
        ...
        result.add_check("排泥周期 T_sludge", 1 <= T_sludge <= 2,
                         T_sludge, "1~2", "d")

        # ── 刮泥机线速 ──
        v_peripheral = PI * D * 1.0 / 60.0  # m/min (1 r/h)
        result.add_check("刮泥机线速", v_peripheral <= 3.0,
                         round(v_peripheral, 2), "<= 3.0", "m/min")
        ...
        self._sludge_output = SludgeFlow(
            Q_wet=S_wet * n, DS=S_dry * n,
            P_moisture=P_sludge, VS_ratio=0.60,
        )
    ...
    def _vectorized_compute(
        ...
        h1 = fixed["h1"]; h3 = fixed["h3"]; i_slope = fixed["i_slope"]
        R1 = fixed["R1"]; R2 = fixed["R2"]; h5 = fixed["h5"]
        P_sludge = fixed["P_sludge"]; T_sludge = fixed["T_sludge"]
        ...
        dtype = np.dtype([
            ("D", np.float64), ("F_actual", np.float64), ("q_prime_actual", np.float64),
            ("h2", np.float64), ("ratio_Dh2", np.float64), ("H_total", np.float64),
            ("h4", np.float64), ("d_center", np.float64),
            ("q_weir", np.float64), ("S_dry", np.float64), ("S_wet", np.float64),
            ("V_total_storage", np.float64), ("v_peripheral", np.float64),
            ("H", np.float64),
            ("concrete_m3", np.float64),
            ("ok_D_min", np.bool_), ("ok_q_prime", np.bool_), ("ok_h2", np.bool_),
            ("ok_Dh2", np.bool_), ("ok_sludge", np.bool_),
            ("ok_weir", np.bool_), ("ok_scraper", np.bool_), ("ok_T_sludge", np.bool_),
            ("val_D_min", np.float64), ("val_q_prime", np.float64), ("val_h2", np.float64), ("val_Dh2", np.float64),
            ("val_sludge", np.float64), ("val_weir", np.float64), ("val_scraper", np.float64),
            ("val_T_sludge", np.float64),
        ])
        result = np.empty(N, dtype=dtype)
        result["D"] = D; result["F_actual"] = F_actual; result["q_prime_actual"] = q_prime_actual
        result["h2"] = h2; result["ratio_Dh2"] = ratio_Dh2; result["H_total"] = H
        result["h4"] = h4_rounded; result["d_center"] = d_center
        result["q_weir"] = q_weir; result["S_dry"] = S_dry; result["S_wet"] = S_wet
        result["V_total_storage"] = V_total_storage; result["v_peripheral"] = v_peripheral
        result["concrete_m3"] = concrete_m3
        result["H"] = result["H_total"]  # standard field
        result["D"] = D  # standard field
        result["ok_D_min"] = ok_D; result["ok_q_prime"] = ok_q; result["ok_h2"] = ok_h2
        result["ok_Dh2"] = ok_Dh2; result["ok_sludge"] = ok_sludge
        result["ok_weir"] = ok_weir; result["ok_scraper"] = ok_scraper
        result["ok_T_sludge"] = ok_T_sludge
        result["val_D_min"] = D; result["val_q_prime"] = q_prime_actual; result["val_h2"] = h2; result["val_Dh2"] = ratio_Dh2
        result["val_sludge"] = V_total_storage - V_sludge
        result["val_weir"] = q_weir; result["val_scraper"] = v_peripheral
