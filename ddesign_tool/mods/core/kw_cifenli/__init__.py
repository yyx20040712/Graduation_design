...
class KwCifenliNode(NodeBase):
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ParamDef("设备台数 N", "N_units", value=2, default=2,
                     min_val=1, max_val=6, step=1, unit="台",
                     description="单台 50~700 m³/h"),
            ParamDef("磁盘直径 D", "D_disk", value=1.0, default=1.0,
                     min_val=0.6, max_val=1.5, step=0.1, unit="m",
                     description="0.6~1.5m,典型1.0m"),
            ParamDef("磁盘间距 δ", "delta", value=0.025, default=0.025,
                     min_val=0.020, max_val=0.030, step=0.005, unit="m",
                     description="20~30mm"),
            ParamDef("浸没率 η", "eta_immerse", value=0.40, default=0.40,
                     min_val=0.35, max_val=0.45, step=0.05, unit="",
                     description="0.35~0.45"),
            ParamDef("转速 ω", "omega", value=3, default=3,
                     min_val=1, max_val=5, step=1, unit="r/min",
                     description="通常2~3 r/min"),
            ParamDef("设计表面负荷", "q_design", value=30, default=30,
                     min_val=20, max_val=40, step=5, unit="m³/(m²·h)",
                     description="20~40 m³/(m²·h)"),
            ParamDef("SS去除率 η_SS", "eta_SS", value=0.95, default=0.95,
                     min_val=0.90, max_val=0.98, step=0.01, unit="",
    ...
    def _default_removal_rates(cls) -> Dict[str, float]:
        return {
            "BOD5": 0.10, "COD": 0.20, "SS": 0.85,
            "NH3N": 0.05, "TN": 0.05, "TP": 0.70,
        }
    ...
    def calculate(self, flow: WaterFlow,
        ...
        result.params = {
            "N_units": N_units, "D_disk": D_disk, "delta": delta,
            "eta_immerse": eta_immerse, "omega": omega, "B0": B0,
            "q_design": q_design, "eta_SS": eta_SS_user,
            "P_sludge": P_sludge, "rho_sludge": rho_sludge,
            "delta_end": delta_end, "delta_side": delta_side,
            "delta_bottom": delta_bottom, "delta_top": delta_top,
        }
        ...
        n_disks_stay = 30 * Q_1_m3s / (PI**2 * D_disk**2 * delta * eta_immerse) + 1 if (D_disk > 0 and delta > 0 and eta_immerse > 0) else 0
        n_disks = max(9, math.ceil(n_disks_calc), math.ceil(n_disks_stay))  # 同时满足表面负荷和停留时间
        ...
        result.add_check("流道停留时间 30~60s",
                         30 <= t_disk <= 60,
                         round(t_disk, 1), "30~60", "s")
        if t_disk < 30:
        ...
        # ── 组装结果 ──
        result.add_dimension("设备台数", N_units, "台",
                             formula="N = 根据总流量确定", category="physical")
        result.add_dimension("单台流量 Q₁", round(Q_1_m3h, 1), "m³/h",
                             formula="Q₁ = Q_max / N", category="computed")
        result.add_dimension("磁盘直径 D", D_disk, "m", category="physical")
        result.add_dimension("磁盘间距 δ", delta * 1000, "mm", category="physical")
        result.add_dimension("磁盘数量 n_disks", n_disks, "盘",
                             formula="n = A_total / A_per_disk, ≥9",
                             category="physical")
        result.add_dimension("总吸附面积 A_total", round(A_total, 1), "m²",
                             formula="A = 2×n×πD²/4×η", category="physical")
        result.add_dimension("流道断面面积 A_ch", round(A_channel, 3), "m²",
                             formula="A_ch = (n-1)×πDδ", category="physical")
        result.add_dimension("流道有效长度 L_ch", round(L_channel, 2), "m",
                             formula="L_ch = η×πD", category="physical")
        result.add_dimension("流道停留时间 t_disk", round(t_disk, 1), "s",
                             formula="t = A_ch×L_ch/Q₁", category="computed")
        result.add_dimension("流道流速 v_disk", round(v_disk, 3), "m/s",
                             formula="v = Q₁/A_ch", category="computed")
        result.add_dimension("实际表面负荷 q", round(q_actual, 1), "m³/(m²·h)",
                             formula="q = Q₁/A_total", category="computed")
        result.add_dimension("外缘线速度 v_line", round(v_line, 3), "m/s",
                             formula="v_line = πD×ω/60", category="computed")
        result.add_dimension("磁场强度 B₀", B0, "T", category="physical")
        result.add_dimension("SS去除率", eta_SS_user * 100, "%",
                             formula="η_SS ≥ 90%", category="computed")
        result.add_dimension("日去除干固体 W_SS", round(W_SS, 2), "t/d",
                             formula="W_SS = Q_total×C₀×η_SS×10⁻⁶", category="computed")
        result.add_dimension("日湿污泥量 V_sludge", round(V_sludge, 1), "m³/d",
                             formula="V = W_SS/((1-P)×ρ)", category="computed")
        result.add_dimension("设备长度 L_machine", round(L_machine, 1), "m",
                             formula="L = n×δ + 2Δ_end", category="physical")
        result.add_dimension("设备宽度 B_machine", round(B_machine, 1), "m",
                             formula="B = D + 2Δ_side", category="physical")
        result.add_dimension("设备高度 H_machine", round(H_machine, 1), "m",
                             formula="H = D + Δ_bottom + Δ_top", category="physical")
        result.add_dimension("单台占地面积", round(S_machine, 1), "m²",
                             formula="S = L × B", category="physical")
        result.add_dimension("总装机功率", round(P_total, 1), "kW",
                             formula="P = P_main + P_scraper", category="computed")

        # 概算用 (设备为主,土建少量)
        result.add_dimension("总占地面积", round(S_machine * N_units, 1), "m²",
                             category="physical")
        result.add_dimension("混凝土量估算",
                           round(S_machine * 0.5 * N_units, 1), "m³",
                             category="physical")
    ...
    def _vectorized_compute(cls, grid: dict, flow: "WaterFlow",
        ...
        if flow.Q_design <= 0:
            dtype = np.dtype([
                ("n_disks", np.int32), ("A_total", np.float64),
                ("A_channel", np.float64), ("L_channel", np.float64),
                ("t_disk", np.float64), ("v_disk", np.float64), ("q_actual", np.float64),
                ("v_line", np.float64), ("L_machine", np.float64), ("B_machine", np.float64),
                ("H_machine", np.float64), ("S_machine", np.float64), ("P_total", np.float64),
                ("W_SS", np.float64), ("V_sludge", np.float64),
                ("H", np.float64), ("area_total", np.float64), ("concrete_m3", np.float64),
                ("ok_tdisk", np.bool_), ("ok_vdisk", np.bool_), ("ok_q", np.bool_),
                ("ok_vline", np.bool_),
                ("val_tdisk", np.float64), ("val_vdisk", np.float64),
                ("val_q", np.float64), ("val_vline", np.float64),
            ])
        ...
        n_disks = np.maximum(9, np.maximum(np.ceil(n_disks_calc), np.ceil(n_disks_stay))).astype(np.int32)
        ...
        result["n_disks"] = n_disks; result["A_total"] = A_total
        result["A_channel"] = A_channel; result["L_channel"] = L_channel
        result["t_disk"] = t_disk; result["v_disk"] = v_disk; result["q_actual"] = q_actual
        result["v_line"] = v_line
        result["L_machine"] = L_machine; result["B_machine"] = B_machine
        result["H_machine"] = H_machine
        result["S_machine"] = S_machine; result["P_total"] = P_total
        result["W_SS"] = W_SS; result["V_sludge"] = V_sludge
        result["H"] = H_machine
        result["area_total"] = area_total; result["concrete_m3"] = concrete_m3
        result["ok_tdisk"] = ok_tdisk; result["ok_vdisk"] = ok_vdisk
        result["ok_q"] = ok_q; result["ok_vline"] = ok_vline
        result["val_tdisk"] = t_disk; result["val_vdisk"] = v_disk
        result["val_q"] = q_actual; result["val_vline"] = v_line
        return result
