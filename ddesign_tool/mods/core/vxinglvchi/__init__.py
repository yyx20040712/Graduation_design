...
class VxinglvchiNode(NodeBase):
    ...
    def calculate(self, flow: WaterFlow, quality: WaterQuality) -> NodeResult:
        ...
        A_actual = L * B
        result.add_dimension(
            "单格长度 L", L, "m", formula="L = ceil(f / B, 0.1m)", category="physical"
        )
        result.add_dimension(
            "单格宽度 B",
            B,
        ...
        result.add_check("单格宽度 B ≤ 5m", B <= 5.0, round(B, 2), "≤ 5.0", "m")
        result.add_check(
            "长宽比 L/B 1~3", 1.0 <= L / B <= 3.0, round(L / B, 2), "1~3", ""
        )
        ...
        W_w = (
            Q_w2 * t_gw * 60.0 + Q_w3 * t_w3 * 60.0 + Q_s * t_bw * 60.0
        ) / 1000.0  # m³
        result.add_dimension(
            "单次冲洗水量 W_w",
            round(W_w, 1),
        ...
        result.add_check(
            "冲洗水占比 η_w < 5%", eta_w < 0.05, round(eta_w * 100, 2), "< 5", "%"
        )
        ...
        result.add_check(
            "V型槽流速 ≤ 0.6 m/s", v_v_slot <= 0.6, round(v_v_slot, 2), "≤ 0.6", "m/s"
        )
        ...
        # 校核: 孔数不宜过少
        result.add_check("扫洗孔数 ≥ 20 个/侧", n_hole >= 20, n_hole, "≥ 20", "个/侧")
        ...
        result.add_check(
            "过滤水头损失 2~3 m", 2.0 <= H_loss <= 3.0, round(H_loss, 2), "2~3", "m"
        )
