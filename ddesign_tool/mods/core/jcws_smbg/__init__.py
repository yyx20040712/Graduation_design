...
from models.base import (
    ...
    ParamDef, Port, PortType, ElevationData,
)
...
class JcwsSmbgNode(NodeBase):
    ...
    def _build_param_defs(self) -> List[ParamDef]:
        return [
            ParamDef("进厂水面标高", "Z_water_inlet", value=100.0, default=100.0,
                     min_val=-10000.0, max_val=10000.0, step=0.001, unit="m"),
            ParamDef("进厂地面标高", "Z_ground", value=102.0, default=102.0,
                     min_val=-10000.0, max_val=10000.0, step=0.001, unit="m"),
            ParamDef("进水管径", "DN_inlet", value=800.0, default=800.0,
                     min_val=300.0, max_val=2000.0, step=50.0, unit="mm"),
        ]
    ...
    def calculate(self, flow: WaterFlow, quality: WaterQuality) -> NodeResult:
        ...
        result.params = {"Z_water_inlet": Z_water, "Z_ground": Z_ground, "DN_inlet": DN * 1000}
    ...
    def _vectorized_compute(cls, grid: dict, flow: WaterFlow,
        ...
        N = 1
        Z_bottom = Z_water - DN
        dtype = np.dtype([("Z_water", np.float64), ("Z_ground", np.float64),
                          ("DN", np.float64), ("Z_bottom", np.float64),
                          ("H", np.float64), ("concrete_m3", np.float64)])
        result = np.empty(N, dtype=dtype)
        result["Z_water"] = Z_water; result["Z_ground"] = Z_ground
        result["DN"] = DN; result["Z_bottom"] = Z_bottom
        result["H"] = 0.0; result["concrete_m3"] = 0.0
        return result
